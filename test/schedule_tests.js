...
function logSchedule(schedule, calculated, expected, showDebug) {
  ...
  if (showDebug) {
    ...
    if (schedule.hasOwnProperty("eachNWeek")) {
      let dayList = schedule.dayOfWeek.reduce(
        (reducer, current) => (reducer = reducer + " " + current),
        ""
      );
      interval = `each ${schedule.eachNWeek} week, on: ${dayList}`;
    }
    if (schedule.hasOwnProperty("month")) {
      let scheduleMonthList = schedule.month.reduce(
        (reducer, current) => (reducer = reducer + " " + current),
        ""
      );
      let scheduleDayList = schedule.day.reduce(
        (reducer, current) => (reducer = reducer + " " + current),
        ""
      );
...
describe("schedule", function () {
  describe("common tests", function () {
    it("valid", function (done) {
      let scheduleTestObject = JSON.parse(
        JSON.stringify(require("./test_data").oneTimeScheduleOK)
      );
    ...
    it("disabled", function (done) {
      ...
      assert.include(
        schedule.nextOccurrence(scheduleTestObject).error,
        "disabled"
      );
      done();
    });
  ...
  describe("summary", function () {
    it("once", function (done) {
      ...
      scheduleTestObject.oneTime = "2119-06-10T02:02:02.071Z";
      assert.equal(
        schedule.summary(scheduleTestObject),
        `Once at ${formatDateTime(parseDateTime(scheduleTestObject.oneTime))}`
      );
      done();
    });
    it("eachNDay", function (done) {
      let scheduleTestObject = JSON.parse(
        JSON.stringify(require("./test_data").dailyScheduleEveryOK)
      );
      ...
      const correct = `Each 3 day(s), every 1 minute(s) between 11:11:11 and 23:59:59, starting ${formatDateTime(
        parseDateTime(scheduleTestObject.startDateTime)
      )} and till ${formatDateTime(
        parseDateTime(scheduleTestObject.endDateTime)
      )}`;
    ...
    it("eachNWeek", function (done) {
      let scheduleTestObject = JSON.parse(
        JSON.stringify(require("./test_data").weeklyScheduleOK)
      );
      const correct = `Each 1 week(s) on Monday, Wednesday and Friday, at 11:11:11, starting ${formatDateTime(
        parseDateTime(scheduleTestObject.startDateTime)
      )}`;
      assert.equal(schedule.summary(scheduleTestObject), correct);
      done();
    ...
    it("month", function (done) {
      let scheduleTestObject = JSON.parse(
        JSON.stringify(require("./test_data").monthlyScheduleOK)
      );
      const correct = `In January and July each 11, 2, 8 and 1 day, every 1 minute(s) between 11:11:11 and 23:59:59, starting ${formatDateTime(
        parseDateTime(scheduleTestObject.startDateTime)
      )}`;
      assert.equal(schedule.summary(scheduleTestObject), correct);
      done();
  ...
  describe("nextOccurrence", function () {
    describe("oneTime", function () {
      it("success. added time", function (done) {
        ...
        scheduleTestObject.oneTime = addDate(
          getDateTime(),
          0,
          0,
          0,
        ...
        assert.equalDate(
          schedule.nextOccurrence(scheduleTestObject).result,
          nextRun
        );
        assert.equalTime(
          schedule.nextOccurrence(scheduleTestObject).result,
    ...
    describe("eachNDay. occursOnceAt", function () {
      it("success. run at now+5min", function (done) {
        let scheduleTestObject = JSON.parse(
          JSON.stringify(require("./test_data").dailyScheduleOnceOK)
        );
        ...
        scheduleTestObject.dailyFrequency.occursOnceAt =
          getTimefromDateTime(nextRunDateTime);
      ...
      it("success. run at 23:59:59", function (done) {
        ...
        scheduleTestObject.startDateTime = addDate(
          getDateTime(),
          0,
          0,
          -15,
      ...
      it("failure. endDateTime restriction", function (done) {
        ...
        assert.equal(
          calculationError,
          "calculated date-time earlier than endDateTime"
        );
        done();
      });
    });
    ...
    describe("eachNDay. occursEvery", function () {
      it("success. run every 15 minutes starting 10:07:00", function (done) {
        ...
        let nextRunDateTime = new Date(
          getDateTime().setUTCHours(time[0], time[1], time[2], 0)
        );
        while (nextRunDateTime < getDateTime()) {
          nextRunDateTime = addDate(
            nextRunDateTime,
            0,
            0,
            0,
            0,
            scheduleTestObject.dailyFrequency.occursEvery.intervalValue,
            0
          );
        }
        //log
      ...
      it('success. run every 10 hours ("end" earlier that next occurrence this day)', function (done) {
        ...
        let time = scheduleTestObject.dailyFrequency.start.split(":");
        nextRunDateTime = new Date(
          addDate(getDateTime(), 0, 0, 1).setUTCHours(
            time[0],
            time[1],
            time[2],
            0
    ...
    describe("eachNWeek", function () {
      ...
      it("success. run every 1st week, everyday, every 2 hours", function (done) {
        ...
        } else nextRunDateTime = addDate(nextRunDateTime, 0, 0, 0, xHour, 0, 0);
      ...
      it("failed. endDateTime restriction", function (done) {
        ...
        scheduleTestObject.dayOfWeek = ["wed", "fri"];
        scheduleTestObject.eachNWeek = 1;
        scheduleTestObject.endDateTime = parseDateTime(
          "2018-10-01T10:00:00.000Z"
        ).toISOString();
        let calculationResult =
          schedule.nextOccurrence(scheduleTestObject).result;
    ...
    describe("month", function () {
      it("success. run every NY", function (done) {
        ...
        let calculationResult =
          schedule.nextOccurrence(scheduleTestObject).result;
        let nextRunDateTime = parseDateTime(
      ...
      it("success. oct 15, 17, 19 once at 07:00", function (done) {
        ...
        let nextRunDateTime = parseDateTime("2022-10-15T07:00:00.000Z");
        logSchedule(scheduleTestObject, calculationResult, nextRunDateTime);
        assert.equalDate(calculationResult, nextRunDateTime);
        assert.equalTime(calculationResult, nextRunDateTime);
        done();
      ...
      it("success. middle of days interval once at 00:01", function (done) {
        ...
        scheduleTestObject.startDateTime = "2018-01-01T10:00:00.000Z";
        scheduleTestObject.month = monthList.slice(
          getDateTime().getUTCMonth(),
          getDateTime().getUTCMonth() + 1
        );
      ...
      it("success. happend 1 minutes ago", function (done) {
        ...
        //1 minutes ago
        scheduleTestObject.dailyFrequency = {
          occursOnceAt: `${leadZero(getDateTime().getUTCHours())}:${leadZero(
            getDateTime().getMinutes() - 1
          )}:${leadZero(getDateTime().getSeconds())}`,
        };
        let calculationResult =
          schedule.nextOccurrence(scheduleTestObject).result;
      ...
      it("success. happen every 13 hours (date overwhelming)", function (done) {
        ...
        scheduleTestObject.dailyFrequency = {
          start: "12:00:00",
          occursEvery: { intervalValue: 13, intervalType: "hour" },
        };
        let calculationResult =
          schedule.nextOccurrence(scheduleTestObject).result;
      ...
      it("success. happen every 13 hours (date overwhelming with next day)", function (done) {
        ...
        scheduleTestObject.day = [
          getDateTime().getUTCDate() + 1,
          getDateTime().getUTCDate(),
        ];
        scheduleTestObject.dailyFrequency = {
          start: "12:00:00",
          occursEvery: { intervalValue: 13, intervalType: "hour" },
      ...
      it("failed. run every NY, endDateTime restriction (5 minutes ago)", function (done) {
        ...
        scheduleTestObject.startDateTime = "2018-01-01T10:00:00.000Z";
        scheduleTestObject.endDateTime = addDate(
          getDateTime(),
          0,
          0,
          0,
